...
from plot_trajectories_run10 import load_mean_std, METRICS, CONSTRAINTS, bootstrap_rho_band, smooth  # noqa: E402
...
def plot_one(root, constraint, stem_key, ylabel, series_fn, hline, out_dir, n_smooth):
    ...
    for agent, _ in AGENTS:
        ...
        for reward in GROUP_REWARDS:
            ...
            if stem_key == "inequality_ratio":
                # same seed-bootstrap band as the PeRL/RL trajectory panels
                import numpy as np
                import pandas as pd
                point, bsd = bootstrap_rho_band(root, agent, reward, constraint)
                if bsd is not None:
                    h = len(point) // 2
                    assert np.allclose(point[h:], mu.values[h:len(point)], rtol=1e-6, equal_nan=True), \
                        f"rho mismatch {agent} {reward} {constraint}"
