...
def parse_filename(filename: str) -> Optional[Dict]:
    ...
    Expected format: pepg_v2_{reward}_{constraint}_seed{N}_{timestamp}_episode_metrics.csv
    where timestamp starts with 20260127
    """
    ...
    if not name.startswith('pepg_v2_'):
        return None
    name = name[len('pepg_v2_'):]
    
    # Remove the _episode_metrics suffix if present
        ...
        if name.startswith(reward + "_"):
            ...
            for constraint in JOB_MATRIX["constraints"]:
                if remaining.startswith(constraint + "_"):
                    ...
                    # Extract seed number and check timestamp
                    seed_match = re.match(r'seed(\d+)_(\d+)', seed_part)
                    if seed_match:
                        seed = int(seed_match.group(1))
                        timestamp = seed_match.group(2)
                        
                        # Check if timestamp starts with 20260127
                        if timestamp.startswith('20260127'):
                            return {
                                'reward': reward,
...
def load_results(directory_path: str) -> Dict:
    ...
    # Find all CSV files
    pattern = os.path.join(directory_path, "pepg_v2_*_episode_metrics.csv")
...
def create_plot_3(aggregated: Dict, constraint: str, output_dir: str, use_log_scale: bool = False, zoom: bool = False):
    ...
    for reward in rewards:
        ...
        # bank_profit_episode = (loans_M - defaults_M + loans_F - defaults_F) * interest_rate * loan_amount - (defaults_M + defaults_F) * loan_amount
        # Simplified: propagate std from loans and defaults
...
def main(directory_path: str, use_log_scale: bool = False, zoom: bool = False):
    ...
    if not results:
        ...
        print("  pepg_v2_{reward}_{constraint}_seed{N}_{timestamp}_episode_metrics.csv")
        return
    ...
    for constraint, table_data in all_table_data.items():
        ...
        results_csv_path = os.path.join(directory_path, f'aggregated_results_{constraint}.csv')
    ...
    for constraint in JOB_MATRIX["constraints"]:
        ...
        if has_data:
            print(f"\n--- Constraint: {CONSTRAINT_LABELS[constraint]} ---")
            create_plot_1(aggregated, constraint, directory_path, use_log_scale, zoom)
            create_plot_2(aggregated, constraint, directory_path, use_log_scale, zoom)
            create_plot_3(aggregated, constraint, directory_path, use_log_scale, zoom)
            create_plot_4(aggregated, constraint, directory_path, use_log_scale, zoom)
...
Examples:
    ...
    python plot_fairness_results.py ./pepg_sequential_results --zoom
    
Expected file format:
    pepg_v2_{reward}_{constraint}_seed{N}_{timestamp}_episode_metrics.csv
...
Output (12 plots total - 4 per constraint):
    ...
    main(args.directory, args.log_scale, args.zoom)
