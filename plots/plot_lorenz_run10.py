...
def plot_one(root, agent, constraint, reward, reward_label, out_dir, gini_rows, draw_eps=None):
    ...
    all_eps = sorted(curves, key=episode_sort_key)
    # The Gini table is written for EVERY snapshot in the data; --episodes
    # only restricts which curves are drawn (and the CSV next to the figure).
    eps = [e for e in all_eps if draw_eps is None or e == "final" or e in draw_eps]
    out = {}
    if curves:
...
def main():
    ...
    ap.add_argument("--episodes", type=str, default="",
                    help="comma-separated snapshot episodes to DRAW (default: all in the data); "
                         "the Gini table always covers every snapshot")
    ap.add_argument("--cells", type=str, default="",
                    help="comma-separated agent:constraint:reward cells to draw instead of the six "
                         "fairness cells, e.g. pepg:dm:utilitarian_profit,pg:dm:utilitarian_profit (MaxUtil)")
    args = ap.parse_args()
    os.makedirs(args.out, exist_ok=True)
    set_paper_style()

    draw_eps = {int(e) for e in args.episodes.split(",") if e.strip()} or None
    gini_rows = []
    all_eps = set()
    if args.cells:
        cells = [tuple(c.split(":")) for c in args.cells.split(",") if c.strip()]
    else:
        cells = [(a, c, r) for a, _ in AGENTS for c, _ in CONSTRAINTS for r, _ in REWARDS]
    labels = dict(REWARDS)
    for agent, constraint, reward in cells:
        eps = plot_one(args.root, agent, constraint, reward, labels.get(reward, reward),
                       args.out, gini_rows, draw_eps)
