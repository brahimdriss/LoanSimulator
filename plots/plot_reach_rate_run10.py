...
def plot_one(root, agent, constraint, reward, out_dir, n_smooth, ymax=None):
    ...
    # Shared y-limit across every panel of a run (--ymax), so the twelve
    # cells and the lambda* panel are read on one scale; 0 .. data max if unset.
...
def main():
    ...
    ap.add_argument("--ymax", type=float, default=None,
                    help="shared upper y-limit for every panel (run11: 0.30 covers all mean+std)")
    args = ap.parse_args()
    ...
    for agent, _ in AGENTS:
        for constraint, _ in CONSTRAINTS:
            for reward, _ in REWARDS:
                plot_one(args.root, agent, constraint, reward, args.out, args.smooth, args.ymax)
