...
def parse_filename(filename: str) -> Optional[Dict]:
    ...
    if not name.endswith('_episode_metrics'):
        return None
...
def print_summary_statistics(aggregated: Dict):
    ...
    for reward in JOB_MATRIX["rewards"]:
        ...
        for constraint in aggregated[reward]:
            ...
            print(f"    Inequality Ratio (rho): {rho:.4f} +/- {rho_std:.4f}")
...
def print_aggregated_by_reward(aggregated: Dict):
    ...
    print(f"\n{'Reward Function':<22} | {'Apr. Rate Disp.':<20} | {'Wealth Gap':<20} | {'Inequality (rho)':<20} | {'N'}")
    print("-" * 100)
...
def main(directory_paths: List[str], output_dir: str, use_log_scale: bool = False):
    ...
    results_csv_path = os.path.join(output_dir, 'aggregated_results_summary.csv')
    results_df.to_csv(results_csv_path, index=False)
    print(f"\nAggregated results saved to: {results_csv_path}")

    # Create plots for each constraint
    print("\n" + "="*80)
    print("GENERATING PLOTS (4 plots x 3 constraints = 12 plots)")
    print("="*80)
    ...
    for constraint in JOB_MATRIX["constraints"]:
        ...
        if has_data:
            ...
            create_plot_1(aggregated, constraint, output_dir, use_log_scale)
            create_plot_2(aggregated, constraint, output_dir, use_log_scale)
            create_plot_3(aggregated, constraint, output_dir, use_log_scale)
            create_plot_4(aggregated, constraint, output_dir, use_log_scale)
...
Output (12 plots total - 4 per constraint):
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="log_scale",
        help="Use symmetric log scale for certain plots"
    )

    args = parser.parse_args()
    main(args.directories, args.output_dir, args.log_scale)
