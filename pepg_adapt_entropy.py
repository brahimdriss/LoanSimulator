...
def _plot_comparison(aggregated, results_dir, timestamp, n_seeds, constraint_filter=None):
    if not any(True for _ in _iter_combos(aggregated, constraint_filter)):
        return
    setup_plot_style()
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    ctitle = PEPG_CONSTRAINT_LABELS.get(constraint_filter, "All") if constraint_filter else "All Constraints"
    fig.suptitle(
        f"Reg PePG Adapt — {ctitle}  (mean ± std, {n_seeds} seeds)",
...
def _plot_social_welfare(aggregated, results_dir, timestamp, n_seeds, constraint_filter=None):
    ...
    if not plot_data:
        plt.close(fig)
        return
    margin = (y_max - y_min) * 0.05 if np.isfinite(y_max - y_min) else 0.1
    ylim = (y_min - margin, y_max + margin) if np.isfinite(y_min) and np.isfinite(y_max) else None
...
def main():
    parser = argparse.ArgumentParser(
        description="Entropy-regularised PePG pre-train → performative deploy on testing env"
    )
    ...
    parser.add_argument("--entropy-coef",    type=float, default=0.01,
                        help="Entropy regularisation coefficient λ (paper Def. 5). "
                             "0 = unregularised PePG.")
    ...
    parser.add_argument("--weights-dir",       type=str, default="./weights_pepg_adapt_entropy")
    parser.add_argument("--results-dir",       type=str, default="./results_pepg_adapt_entropy")
    ...
    print("=" * 70)
    print("ENTROPY-REGULARISED PEPG PRE-TRAIN → PERFORMATIVE DEPLOY")
    print("=" * 70)
    print(f"  Seeds           : {seeds}")
