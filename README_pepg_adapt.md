# `pg_run.py` — PePG Performative Pre-train → Frozen Deploy

Trains a **Performative Policy Gradient (PePG)** agent on a performative lending environment, then deploys it with no updates in a persistent testing environment.
...
python pg_run.py \
  ...
  --warmup 20 \
  --buffer-capacity 50 \
  --weights-dir /content/drive/MyDrive/Long-term-Fairness-NeurIPS/weights_pg \
  --results-dir /content/drive/MyDrive/Long-term-Fairness-NeurIPS/results_pg \
  --seeds 3
```
...
Trains a **Performative Policy Gradient (PePG)** agent on a performative lending environment, then deploys it with continued performative updates in a persistent testing environment.
